      ...
      .then(([fetchedClients, fetchedInactif]) => {
        if (filter) {
          const flatFilter = Array.isArray(filter[0])
            ? filter.flat()
            : filter.map((item: { matricule: any }) => item.matricule);
          const filteredClients = fetchedClients.filter(
            (client: { matricule: any }) =>
    ...
    useEffect(() => {
      ...
      Promise.all([
        ...
        Promise.all(responses.map((response) => response.json()))
          .then(([fetchedClients, fetchedFiltersList]) => {
            setClients(fetchedClients);
      ...
      <div className="flex justify-between">
        ...
        <DeleteIcon
          ...
          onClick={(e) => {
            ...
            if (
              window.confirm('Êtes-vous sûr de vouloir supprimer ce filtre?')
            ) {
                    ...
                    if (newValue) {
                      try {
                        ...
                        if (response.ok) {
                          ...
                          const filteredClients = cleanClients.filter(
                            (client) => {
